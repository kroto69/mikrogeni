...
function InfoItem({ label, value, action, muted = false }: { label: string; value: ReactNode; action?: ReactNode; muted?: boolean }) {
  return (
    <div className="space-y-1.5 rounded-2xl bg-card/80 p-3 sm:p-4 dark:bg-card/70">
...
function SkeletonCard({ className }: { className?: string }) {
  return <div className={cn("animate-pulse rounded-3xl bg-card/70 dark:bg-card/60", className)} />;
}
...
function OverlayPanel({
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-end justify-center bg-slate-950/35 p-3 sm:items-center sm:p-6">
      ...
      <div className="relative z-10 max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-t-[28px] border border-border bg-card text-card-foreground shadow-2xl sm:rounded-[28px] dark:bg-card/95">
...
export default function OnuDetail() {
  ...
  const renderParameterRows = (
    ...
    <div className="space-y-4">
      {rows.map((row, index) => (
        <div className="grid gap-3 rounded-2xl border border-border/80 p-4 sm:grid-cols-[1.3fr_1fr_0.8fr_auto]" key={row.id}>
          <Input
  ...
  const renderMoreActionButtons = () => (
    <>
      <button
        className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm text-foreground hover:bg-card/80 dark:hover:bg-card/70"
        onClick={() => {
          setShowMoreActions(false);
      ...
      <button
        className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm text-foreground hover:bg-card/80 dark:hover:bg-card/70 disabled:opacity-50"
        disabled={detailQuery.isFetching || summonDeviceMutation.isPending}
        onClick={handleRefreshDetail}
        type="button"
      >
        <RefreshCcw className="h-4 w-4" /> Refresh detail
      </button>
    </>
  );
  ...
  if (!id) {
    return (
      <Card>
        <CardContent className="p-6 text-sm text-rose-600">Missing ONU device id.</CardContent>
      </Card>
    );
  }
  ...
  return (
    <>
      <div className="mx-auto max-w-[22.5rem] space-y-4 pb-24 md:max-w-none md:space-y-6 md:pb-0">
        <div className="flex items-center justify-between rounded-3xl border border-border bg-card/90 px-3.5 py-2.5 shadow-sm md:hidden dark:bg-card/80">
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div className="min-w-0 space-y-3">
            <div className="hidden flex-wrap items-center gap-2 text-xs text-muted-foreground md:flex">
              <button
                className="inline-flex items-center gap-2 rounded-2xl border border-border bg-card/95 px-3 py-2 text-sm font-medium text-foreground transition hover:bg-card/90 dark:bg-card/90 dark:hover:bg-card/80"
                onClick={() => {
                  if (window.history.length > 1) {
                    navigate(-1);
                    return;
            ...
            <div className="rounded-[28px] border border-border bg-card/90 p-3.5 shadow-[0_20px_48px_-38px_rgba(15,23,42,0.32)] md:border-0 md:bg-transparent md:p-0 md:shadow-none dark:bg-card/80">
              <div className="flex items-center gap-2 md:hidden">
                <h2 className="text-[clamp(1.45rem,5.4vw,1.85rem)] font-semibold tracking-tight text-foreground">{detail.vendor} {detail.device_type}</h2>
                <Badge variant="success">Active</Badge>
              </div>
              <h2 className="hidden break-all text-2xl font-semibold text-foreground sm:text-3xl md:block">{detail.serial_number}</h2>
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1.55fr)_320px] xl:gap-6">
          <div className="space-y-5 xl:space-y-6">
            <Card className="border-border/60 bg-card/85 shadow-[0_24px_60px_-42px_rgba(15,23,42,0.35)] dark:bg-card/80">
              <CardContent className="space-y-4 p-3.5 sm:space-y-6 sm:p-6">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <div className="hidden flex-wrap items-center justify-end gap-2 md:flex">
                    ...
                    <ActionButton disabled={isActionPending} icon={<Globe2 className="h-4 w-4" />} label="Config WAN" onClick={() => setActiveModal("wan")} />
                    ...
                    {showMoreActions ? (
                    <div className="absolute right-0 top-full z-20 mt-2 w-52 rounded-2xl border border-border bg-card/95 p-2 shadow-xl dark:bg-card/90">
                ...
                <div className="grid grid-cols-3 gap-2 sm:grid-cols-3 sm:gap-3 xl:grid-cols-4">
                  {[
                    ...
                    <div className="rounded-2xl bg-card/85 px-2.5 py-2.5 shadow-sm ring-1 ring-border/60 sm:px-3 sm:py-3 dark:bg-card/80" key={label}>
                      <div className="mb-1.5 flex justify-center md:justify-start">
                        {index === 0 ? <Clock3 className="h-3.5 w-3.5 text-sky-700 sm:h-4 sm:w-4" /> : null}
                        {index === 1 ? <Thermometer className="h-3.5 w-3.5 text-sky-700 sm:h-4 sm:w-4" /> : null}
                        {index === 2 ? <Radio className="h-3.5 w-3.5 text-sky-700 sm:h-4 sm:w-4" /> : null}
                      </div>
                      ...
                      <div className="mt-1 flex flex-col items-start gap-1 md:flex-row md:items-center md:gap-2">
                        <p className={cn(
                          ...
                          index === 1 && tempBadge.variant === "warning" && "text-amber-600",
                          index === 1 && tempBadge.variant === "destructive" && "text-rose-600",
                          index === 2 && rxBadge.variant === "warning" && "text-amber-600",
                          index === 2 && rxBadge.variant === "destructive" && "text-rose-600",
                        )}>
                          {value}
                        </p>
            ...
            <Card className="border-border/60 shadow-[0_20px_52px_-40px_rgba(15,23,42,0.3)]">
              <CardHeader>
              ...
              <div className="rounded-2xl border border-dashed border-border px-4 py-10 text-center text-sm text-muted-foreground">No WiFi profiles available</div>
                  ...
                  <div className="grid gap-2.5 md:grid-cols-2">
                    {detail.wifi_profiles.map((profile: WifiProfile) => (
                      <div className="flex flex-col gap-2.5 rounded-2xl border border-border/60 bg-card/80 px-3 py-3 shadow-[0_10px_24px_-24px_rgba(15,23,42,0.28)] sm:flex-row sm:items-center sm:justify-between dark:bg-card/70" key={profile.index}>
                        <div className="min-w-0 space-y-1">
                          <div className="flex items-center gap-2">
                            <span className="inline-flex h-8 w-8 items-center justify-center rounded-xl bg-emerald-50 text-emerald-700"><Wifi className="h-3.5 w-3.5" /></span>
                        ...
                        <div className="flex flex-wrap items-center gap-2.5 sm:justify-end">
                          ...
                          <div className="flex items-center gap-2 rounded-xl bg-card/80 px-2.5 py-1.5 text-[13px] text-foreground dark:bg-card/70">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-card/85 px-3.5 py-2.5 dark:bg-card/70">
                  <div>
            ...
            <Card className="border-border/60 shadow-[0_20px_52px_-40px_rgba(15,23,42,0.3)]">
              ...
              <CardContent>
                {detail.client_list.length === 0 ? (
                  <div className="rounded-[28px] border border-dashed border-border/60 bg-card/80 px-4 py-14 text-center dark:bg-card/70">
                    <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-sky-200 text-sky-700">
          ...
          <div className="space-y-6">
            <Card className="border-border/60 shadow-[0_20px_50px_-42px_rgba(15,23,42,0.35)]">
              <CardHeader>
            ...
            <Card className="border-border/60 bg-card/85 shadow-[0_20px_50px_-42px_rgba(37,99,235,0.28)] dark:bg-card/80">
              <CardHeader>
            ...
            <Card className="border-border/60 bg-sky-950 text-white shadow-[0_24px_60px_-40px_rgba(2,6,23,0.65)]">
              <CardContent className="space-y-4 p-5">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-sky-200">Diagnostics</p>
                ...
                <div className="flex flex-wrap gap-2">
                  <Button className="bg-sky-200 text-sky-950 hover:bg-sky-300" disabled={isActionPending} onClick={() => setActiveModal("parameter")} type="button">
                    Run Diagnostics
                  </Button>
                  <Button className="border-sky-200/70 text-white hover:bg-sky-900/70" disabled={isActionPending} onClick={() => setActiveModal("security")} type="button" variant="outline">
                    Secure Device
                  </Button>
                </div>
              </CardContent>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 border-t border-border bg-card/95 p-3 backdrop-blur dark:bg-card/90 md:hidden">
        <div className={cn("grid gap-2", isIncomplete ? "grid-cols-5" : "grid-cols-4")}>
          {isIncomplete ? (
            <button className="flex flex-col items-center gap-1 rounded-2xl border border-sky-200 bg-sky-200/80 px-2 py-3 text-[11px] font-semibold text-sky-950 disabled:opacity-50" disabled={isActionPending} onClick={() => void summonDeviceMutation.mutateAsync()} type="button">
              <RefreshCcw className="h-4 w-4" /> Summon
            </button>
          ) : null}
          <button className="flex flex-col items-center gap-1 rounded-2xl bg-rose-600 px-2 py-3 text-[11px] font-semibold text-white disabled:opacity-50" disabled={isActionPending} onClick={() => setActiveModal("reboot")} type="button">
            <Power className="h-4 w-4" /> Reboot
          </button>
          <button className="flex flex-col items-center gap-1 rounded-2xl border border-border bg-card/90 px-2 py-3 text-[11px] font-semibold text-foreground disabled:opacity-50 dark:bg-card/80" disabled={isActionPending} onClick={openWifiModal} type="button">
            <Wifi className="h-4 w-4" /> WiFi
          </button>
          <button className="flex flex-col items-center gap-1 rounded-2xl border border-border bg-card/90 px-2 py-3 text-[11px] font-semibold text-foreground disabled:opacity-50 dark:bg-card/80" disabled={isActionPending} onClick={() => setActiveModal("wan")} type="button">
            <Globe2 className="h-4 w-4" /> WAN
          </button>
          <button className="flex flex-col items-center gap-1 rounded-2xl border border-border bg-card/90 px-2 py-3 text-[11px] font-semibold text-foreground disabled:opacity-50 dark:bg-card/80" disabled={isActionPending} onClick={() => setShowMoreActions((current) => !current)} type="button">
            <MoreHorizontal className="h-4 w-4" /> More
          </button>
        </div>
        {showMoreActions ? (
          <div className="mt-2 grid gap-2 rounded-2xl border border-border bg-card/95 p-2 shadow-xl dark:bg-card/90">
            {renderMoreActionButtons()}
          </div>
        ) : null}
      </div>
      ...
      <OverlayPanel open={activeModal === "reboot"} title="Reboot Device" description="Yakin ingin reboot perangkat ini?" onClose={() => setActiveModal("none")}>
        <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
          ...
          <Button className="w-full bg-rose-600 text-white hover:bg-rose-700 sm:w-auto" disabled={isActionPending} onClick={() => void rebootTask.trigger()} type="button">
            Confirm Reboot
          </Button>
        </div>
      </OverlayPanel>
      ...
      <OverlayPanel open={activeModal === "wifi"} title="Config WiFi" description="Select an existing SSID profile, adjust the name or password, then save." onClose={() => setActiveModal("none")}>
        <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); void handleWifiSubmit(); }}>
          <div className="grid gap-4 sm:grid-cols-2">
            <select
              className="h-11 rounded-xl border border-input bg-background px-3 text-sm text-foreground shadow-sm sm:col-span-2"
            ...
            <div className="flex h-11 items-center rounded-xl border border-input bg-card/80 px-3 text-sm font-medium text-foreground shadow-sm dark:bg-card/70">
              {wifiForm.band}
            </div>
            <select
              className="h-11 rounded-xl border border-input bg-background px-3 text-sm text-foreground shadow-sm"
